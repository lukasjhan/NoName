/* filename          /Kernel64/Source/MPConfigurationTable.c
 * date              2018.12.11
 * last edit date    2018.12.11
 * author            NO.00[UNKNOWN]
 * brief             source code for MP config table
*/
...
void kPrintMPConfigurationTable( void )
{
    ...
    for ( i = 0 ; i < pstMPTableHeader->wEntryCount ; i++ )
    {
        ...
        case MP_ENTRYTYPE_IOINTERRUPTASSIGNMENT:
            ...
            kPrintf( "(%s, %s)\n", vpcInterruptFlagsPO[ pstIOAssignmentEntry->wInterruptFlags & 0x03 ], vpcInterruptFlagsEL[ ( pstIOAssignmentEntry->wInterruptFlags >> 2 ) & 0x03 ] );
